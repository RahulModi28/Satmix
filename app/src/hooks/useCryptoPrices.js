...
const POLL_INTERVAL = 10_000 // 10 seconds — fast enough for a live-feeling demo

/**
 * Custom hook that fetches live crypto prices from CoinGecko REST API.
 * Provides INR prices directly — no backend server needed.
 * Works globally on Vercel without any separate infrastructure.
 *
...
export default function useCryptoPrices() {
    ...
    const fetchPrices = useCallback(async () => {
        try {
            ...
            if (!res.ok) {
                // 429 = rate-limited — keep showing stale data silently
            ...
            data.forEach(coin => {
                ...
                newPrices.set(meta.symbol, {
                    ...
                    name: meta.name,
                    symbol: meta.symbol,
                    price_usd: coin.current_price / 85,
